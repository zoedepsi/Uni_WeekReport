<template>
  <div class="page-wrapper">
    <div class="page-title">未处理回收申请</div>
    <div class="bills-wrapper">
      <el-table :data="tableData" stripe style="width: 100%">
        ...
        <el-table-column label="操作" prop="status">
          <template slot-scope="scope">
            <el-button :disabled="tableData[scope.$index].status!='申请中'" @click="clickPass(scope.$index)" type="text"
                       size="small">通过
            </el-button>
            <el-button :disabled="tableData[scope.$index].status!='申请中'" @click="clickRefuse(scope.$index)" type="text"
                       size="small">拒绝
            </el-button>
          </template>
        </el-table-column>
...
<script type='text/ecmascript-6'>
  ...
  export default {
    ...
    created() {
      ...
      this.axios({
        method: 'post',
        url: rootPath + '/dbs/recycling/getNoDealRecyclingApply',
        params: {
          adminId: "*",
        }
      }).then((response) => {
        if (response.data && response.data.isSucceed && response.data.returnData.bizIsSucceed) {
          var data = response.data.returnData.bizReturnData.nodealrecyclingapplylist;
